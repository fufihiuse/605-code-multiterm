...
public class PolyRoot {
    ...
    private static double newtonsMethod(int[] poly, double x0, int iter) {
        double evalResult;
        /* Evaluate polynomial as f(x0) and
        * check if answer is close or past max iterations.
        * Checking MAX_ITERATIONS first will prevent unnecessary work being
        * done once hitting MAX_ITERATIONS
        */
        if(iter == MAX_ITERATIONS || Math.abs(evalResult = PolyEval.evaluate(poly, x0)) <= EPSILON)
            return x0;

        // Calculate f'(x0)
        int[] deriv = PolyDerive.computeDerivative(poly);
        double derivResult = PolyEval.evaluate(deriv, x0);

        // Recursively call method until condition 2 is met
        return newtonsMethod(poly,x0 - (evalResult / derivResult), ++iter);
    }
}
